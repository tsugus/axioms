...
の省略記法であり、「ただ一つの \(x\) に対して \(P(x)\) である」を意味する。
...
\begin{enumerate}
  ...
  \item\label{260508071601} \(S(x)\land S(y)\rightarrow\exists!z(x+z=y)\)
  ...
  \item \(S(x)\land S(y)\rightarrow(x=y\rightarrow\lnot x>y\land\lnot y>x)\)
...
公理に基づいて、負数 \(-x,\) 差 \(x-y,\) 商 \(x/y,\) 自乗 \(x^2,\) 正の平方根 \(\sqrt{x}\) などがただ一つ存在することが示され、定義可能になる。

公理 \ref{260425093802} は公理ではなく\textbf{公理図式}\index{こうりずしき@公理図式}であり、実際は無数の論理式である。
しかし、アルゴリズム的に生成あるいは機械的に判別可能なものなので、「有限の立場」に反しない。

ちなみに、公理 \ref{260425093801} と \ref{260425093802} をひとまとめにして
\[\exists x(S(x)\land\varphi_n(x)=0)\]
であるとすると、スカラーの全体が複素数体になってしまい、大小関係を一般的に定義することができなくなる。
...
\begin{enumerate}
  ...
  \item\label{260425093803} \(\displaystyle S(z)\land V(x,y,w)\rightarrow z+w=w+z=\left(1+\frac{z}{\sqrt{x^2+y^2}}\right)\cdot w\)
  ...
  \item \(V(x,y,z)\land V(u,v,w)\rightarrow z\cdot w=x\cdot u+y\cdot v\)
\end{enumerate}
...
定義 \ref{260419033901} の記法を用い、変項がスカラーを表すとして公準の本質的な部分を書き換えると、こうなる。
...
この公理系と諸公準は、\(S(x)\) を「\(x\) は実数である」、\(V(x,y,z)\) を「\(z\) はベクトル \(\langle x,y\rangle\) である」と解釈し、\(+\) を和、\(\cdot\) を積、ただし公準 \ref{260425093806} のそれは内積、と解釈し、実数と二次元の実ベクトルのみを含む集合を対象領域とすれば、公準 \ref{260425094701} 以外すべて成り立つ。
...
三角形は一直線上にない三点と同一視され、角は三角形のそれ、もしくは、大きさゼロか \(180\) 度のものに限られる。
角の大小は、角を挟む一方の辺と角の対辺のそれぞれを重ね合わせた場合における対辺の長短と同一視される。


\section{二次元ユークリッド空間であることの確認}

以下においては、\(S(x)\) を満たすもの、すなわち\textbf{スカラーをギリシャ文字の小文字で表し}、そうでないもの、すなわち\textbf{ベクトルをラテン文字の小文字で表す}。
ベクトルは点と同一視される。
...
\begin{proof}
  \(-\) の定義（未記載）と公理 \ref{260508071601} より明らか。
\end{proof}

% 定理 3

\begin{thm}\label{260527103901}
  任意の点 \(x\) と \(a\) に対して、
  \[\overrightarrow{xy}=a\]
  となる点 \(y\) がただ一つ存在する。
\end{thm}
...
\begin{dfn}[\textbf{角の合同}\index{ごうどう@合同（角の）}]
  任意の点 \(x,y,z\) と \(u,v,w\) に対して、
  スカラー \(\lambda>0,\ \mu>0\) が存在して
  \begin{gather*}
    \|\overrightarrow{yx}\|=\|\lambda\overrightarrow{vu}\|\neq 0,\quad\|\overrightarrow{yz}\|=\|\mu\overrightarrow{vw}\|\neq 0, \\
    \|\overrightarrow{yx}-\overrightarrow{yz}\|=\|\lambda\overrightarrow{vu}-\mu\overrightarrow{vw}\|
  \end{gather*}
  となることを、角 \(xyz\) は角 \(uvw\) と合同であるといい、
  \[\angle xyz\equiv\angle uvw\]
  と表す。
\end{dfn}
...
\begin{thm}\label{260424155001}
  ...
  任意のスカラー \(\lambda\geq 0,\ \mu\geq 0\) に対して
...
\begin{col*}
  定理 \ref{260424155001} により、角の合同が反射律・対称律だけでなく推移律も満たすことがわかる。
\end{col*}

% 定義 5

\begin{dfn}[\textbf{角の大小}\index{だいしょう@大小（角の）}]
  任意の点 \(x,y,z\) と \(u,v,w\) に対して、
  \[\angle xyz\equiv\angle x'y'z',\quad \angle uvw\equiv\angle u'y'z'\]
  となる点 \(x',y',z',u'\) が存在し、
    スカラー \(\lambda\geq 0,\ \nu>1\) が存在して
  \[\overrightarrow{y'x'}-\overrightarrow{y'z'}=\nu(\lambda\overrightarrow{y'u'}-\overrightarrow{y'z'})\]
  となることを、角 \(xyz\) は角 \(uvw\) よりも大きいといい、
  \[\angle xyz>\angle uvw\]
  と表す。
\end{dfn}
...
また、省略表現として、角 \(abc\)（ただし、これは独立した項ではないことに注意）を角 \(b\) といい、\(\angle abc\) を \(\angle b\) と書くことがある。
