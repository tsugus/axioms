...
\begin{dfn}[\textbf{直線上}\index{ちょくせんじょう@直線上}]
  点 \(x\) は直線 \(ab\) の上にある、を
  \[\exists\lambda\{x-a=\lambda(b-a)\} \quad (a\neq b)\]
  と定義する。
  点 \(x\) が直線 \(yz\) の上にあることを、\(y,x,z\) は\textbf{一直線上}\index{いっちょくせんじょう@一直線上}にある、ともいう。また、このことを、直線 \(yz\) は点 \(x\) を\textbf{通る}\index{とおる@通る}、ともいう。
\end{dfn}
...
\begin{dfn}[\textbf{半直線上}\index{はんちょくせんじょう@半直線上}]
  点 \(x\) は半直線 \(ab\) の上にある、を
  \[\exists\lambda\{x-a=\lambda(b-a)\land\lambda\geq 0\} \quad (a\neq b)\]
  と定義する。
\end{dfn}
...
\begin{dfn}[\textbf{半直線の延長上}\index{はんちょくせんのえんちょうじょう@半直線の延長上}]
  ...
  \[\exists\lambda\{x-a=\lambda(b-a)\land\lambda<0\} \quad (a\neq b)\]
  と定義する。
\end{dfn}
...
\begin{dfn}[\textbf{線分上}\index{せんぶんじょう@線分上}]
  点 \(x\) は線分 \(ab\) の上にある、を
  \[\exists\lambda\{x-a=\lambda(b-a)\land0\leq\lambda\leq 1\} \quad (a\neq b)\]
  と定義する。
\end{dfn}
...
\begin{dfn}[\textbf{直線図形の合同}\index{ごうどう@合同（直線図形の）}]
...
\begin{col*}
  直線・半直線・線分の合同は、同値関係の性質を引き継ぐので、明らかに反射律、対称律、推移律が成り立つ。
  また、「点 \(z\) は直線 \(xy\) 上にある」かつ「直線 \(xy\) と直線 \(uv\) は合同」ならば、「点 \(z\) は直線 \(uv\) 上にある」。
\end{col*}

% 定義 12

\begin{dfn}[\textbf{関数 \(d\)}\index{でぃー@\(d\)（関数）}・\textbf{長さ}\index{ながさ@長さ}]
...
\begin{dfn}[\textbf{半平面上}\index{はんへいめん@半平面上}・\textbf{へり}\index{へり@へり（半平面の）}・\textbf{同じ側}\index{おなじがわ@同じ側（直線に関して）}]
  点 \(a,b,c\) が一直線上にないとき、点 \(x\) は半平面 \(cab\) の上にある、を
  \[\exists\lambda\mu\{x-a=\lambda(b-a)+\mu(c-a)\land\mu\geq 0\}\]
  と定義する。
  点 \(x\) は直線 \(ab\) をへりとし点 \(c\) と同じ側の半平面の上にある、ともいう。
  また、このとき \(\mu\neq 0\) ならば、点 \(x\) と点 \(c\) は直線 \(ab\) に関して同じ側にある、という。
\end{dfn}

% 定義 14

\begin{dfn}[\textbf{共役な半平面上}\index{きょうやくなはんへいめん@共役な半平面上}・\textbf{へり}\index{へり@へり（共役な半平面の）}・\textbf{反対側}\index{はんたいがわ@反対側（直線に関して）}]\label{260502235601}
  点 \(a,b,c\) が一直線上にないとき、点 \(x\) は共役な半平面 \(cab\) の上にある、を
  \[\exists\lambda\mu\{x-a=\lambda(b-a)+\mu(c-a)\land\mu\leq 0\}\]
  と定義する。
  点 \(x\) は直線 \(ab\) をへりとし点 \(c\) と反対側の半平面の上にある、ともいう。
...
\begin{thm}\label{360424013601}
  与えられた点と同じ側にある点とを結ぶ線分は、与えられた直線と交わらない。
\end{thm}
...
\begin{proof}
  ...
  さて、線分 \(cp\) 上の、点 \(c\) と異なる任意の点 \(x\) について、
  \[x-c=\nu(p-c),\quad 0<\nu\leq 1.\]
  また
  \[a=\nu a+(1-\nu)a\]
  であるから、
...
\begin{thm}\label{260424012501}
  与えられた点と反対側にある点とを結ぶ線分は、与えられた直線と両端以外のある点で交わる。
\end{thm}

\begin{proof}
  点 \(p\) が共役な半平面 \(cab\) 上にあって、直線 \(ab\) 上にないとする。
...
\begin{proof}
  ...
  定理 \ref{360424013601} と定理 \ref{260424012501}より、\(b\) と \(c\) は直線 \(de\) の互いに反対側にある。
  そして、定理 \ref{260424194701} と、半平面および共役な半平面の定義より、頂点 \(a\) は、直線 \(de\) 上にあるか、さもなければ \(de\) をへりとする二つの半平面のどちらか一方の上にある。
...
\begin{proof}
  \begin{align*}
    x\cdot(y-y_0) &=x\cdot\left(y-\frac{x\cdot y}{\|x\|^2}x\right)=x\cdot y-x\cdot\frac{x\cdot y}{\|x\|^2}x \\
...
\begin{dfn}[\textbf{三角形の合同}\index{ごうどう@合同（三角形の）}]
...
\begin{col*}
  この体系においては、三角形の角の合同は頂点に対する辺（すなわち線分）の等長を意味するので、この定義は実質的に辺の等長のみで成立する。
\end{col*}
...
\begin{proof}
  半直線 \(ab\) 上の点 \(x\) を表す式 \(x-a=\lambda(b-a)\ (\lambda\geq 0)\) において \(\displaystyle \lambda=\frac{\overline{cd}}{\|b-a\|}\) とおくと、
...
\begin{proof}
  ...
  ゆえに、直線のベクトルに垂直なベクトル \(n\) を選べば、定理 \ref{260527103901} より、与えられた点から出るベクトルが存在する。
\end{proof}

% 準公理 1

\begin{semi}[三角形の移動の公理]
  半直線とその上にも延長上にもない点および三角形が与えられたとき、
  半直線の上に点をとり、さらに、半直線とその延長をへりとする半平面のうち、与えられた点と同じ側の上に点をとり、与えられた三角形と合同な三角形を作ることができる。
\end{semi}

\begin{proof}
  与えられた三点を \(a,b,c\) とし、三角形 \(xyz\) が与えられているとする。

  \(\overrightarrow{yz}\) の上への \(\overrightarrow{yx}\) の正射影を \(\overrightarrow{yx_0}\) とすると、定理 \ref{260427130101} より
  \[\overrightarrow{yz}\cdot\overrightarrow{x_0x}=\overrightarrow{yz}\cdot(\overrightarrow{yx}-\overrightarrow{yx_0})=0.\]
  ...
  さて、補題 \ref{260424041702} により、半直線 \(bc\) 上に \(\overline{ba_0}=\overline{yx_0}\) となる点 \(a_0\) と、\(\overline{bc'}=\overline{yz}\) となる点 \(c'\) をとり、補題 \ref{260424041703} により、\(a_0\) から出る垂直なベクトルを立てる。
  そして、そのベクトルによって定まる直線の上の点として \(\overline{a_0a'}=\overline{x_0x}\) となる点 \(a'\) を、ただし直線 \(bc\) に関して点 \(a\) と同じ側にあるようにとれば、
  ...
  ゆえに
  \[\overline{a'b}=\overline{xy},\quad \overline{a'c'}=\overline{xz}.\]
  \(\overline{bc'}=\overline{yz}\) は既出である。
  よって、\(\triangle a'bc'\equiv\triangle xyz.\)
\end{proof}
...
\begin{proof}
  ...
  異なる二点 \(a,b\) を通る直線は、それがどのような直線、例えば直線 \(xy\) であれ、\(a,b\) は直線 \(xy\) 上にあるのだから、補題 \ref{260424111401} により直線 \(ab\) と直線 \(xy\) は合同である。

  そして、直線の合同が推移律を満たすことより、異なる二点 \(a,b\) を通る直線はすべて合同である。

  直線が存在することに関しては、この体系において直線は対象物でないので問題を正面から扱うことができないが、「その直線の上の点が存在する」と解すれば、直線は二点で規定され、少なくともその二点を通るのだから、自明である。
\end{proof}
...
\begin{proof}
  左辺の自乗を展開して定理 \ref{260424234101} を用いれば示せる。
\end{proof}

% 補題 8

\begin{lmm}\label{260424225401}
  \(x\neq y,\ z\neq y\) である任意の三点 \(x,y,z\) について、
...
\begin{proof}
  異なる二直線のベクトル \(\overrightarrow{ab},\overrightarrow{cd}\) が平行でないとし、\(\overrightarrow{ae}=\overrightarrow{cd}\) となる点 \(e\) をとる。
...
\begin{semi}[平行線の唯一性の公理]
  与えられた直線の上にない一点を通ってその直線に交わらない直線は高々一つ。
\end{semi}

\begin{proof}
  その直線外の一点が合同でない二つの直線の上にあったとしよう。
  ...
  ところが、この等式を用いると一方の直線の上の点が必ず他方の直線の上にもあることが分かるので、合同でない二つの直線という前提に反する。
\end{proof}
